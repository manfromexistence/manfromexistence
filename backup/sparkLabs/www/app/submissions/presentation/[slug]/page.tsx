...
function Page({ params }: { params: { slug: string } }) {
  ...
  return (
    <>
      ...
      <Link href="/dashboard" className="flex items-center justify-start fixed top-0 left-1.5 w-[152.25px] h-[28.25px] z-50">
      ...
      <div className='fixed z-50 top-10 left-5 flex items-start justify-center w-[340px] bg-background rounded-md border'>
        <div className='rounded-md p-3 w-full !text-sm'>
          {/* <span className='font-mono text-start w-full font-bold text-md'>Project Details</span> */}
          {submissions.map((items: any) => items.id === params.slug && <div key={items.id}>
            ...
            <div className="flex items-center justify-start gap-2 py-1.5 hover:bg-primary-foreground text-lg font-bold">
              <p className="flex flex-row text-center">Project Name: </p>
              <span className="w-auto select-all text-start">{items.title || "No project name is provided"}</span>
            </div>
            <Separator />
            <div className="flex items-center justify-start gap-2 py-1.5 hover:bg-primary-foreground">
              <p className="flex flex-row text-center">Classroom Name: </p>
              {classrooms.map((classroom: any) => classroom.id === items.classroomId && <span key={classroom.id} className="w-auto select-all text-start">{classroom.title || "No classroom name provided"}</span>)}
            </div>
            <Separator />

            <div className="flex items-center justify-start gap-2 py-1.5 hover:bg-primary-foreground">
              <p className="flex flex-row text-center">Date Created: </p>
              <span className="w-auto select-all text-start text-sm">{items.time || "No Time is Provided."}</span>
            </div>
            <Separator />
            {/* <div className="flex items-center justify-start gap-2 py-1.5 hover:bg-primary-foreground">
              <p className="flex flex-row text-center">Teacher Name:</p>
              {classrooms.map((classroom: any) => {
                const user = users.find((u: any) => u.id === classroom.userId);

                // alert(user.id)
                // alert(classroom.userId)

                return user ? (
                  <span key={classroom.id} className="w-auto select-all text-start">
            ...
            <div className="flex items-center justify-start gap-2 py-1.5 hover:bg-primary-foreground">
              <p className="flex flex-row text-center">Teacher Name:</p>
              {classrooms.map((classroom: any) => classroom.id === items.classroomId && users.map((user: any) => user.id === classroom.userId && <span key={user.id} className="w-auto select-all text-start">fkldsfj;askdf</span>))}

              {classrooms.map((classroom: any) => {
                if (classroom.id === items.classroomId) {
                  const teacher = users.find((user: any) => user.userId === classroom.userId);
                  return (
                    <span key={classroom.id} className="w-auto select-all text-start">
                      {teacher ? teacher.username.toUpperCase() : 'No techer name found'}
                      {/* {`classroom Id:${classroom.id} Classroom Title:${classroom.title} classroom.userId ${classroom.userId} user.id${teacher.id} user.userId:${teacher.userId}`} */}
                    </span>
                  );
                }
                return null; // No matching classroom
              })}
            </div>
            ...
            {/* <div className="flex items-center justify-start gap-2 py-1.5 hover:bg-primary-foreground">
              ...
              {classrooms.map((classroom: any) => {
                // classroom.id === items.classroomId && users.map((user: any) => user.id === classroom.userId && <span key={classroom.id} className="w-auto select-all text-start">{user.username.toUpperCase() || "No teahcer found."}</span>)
                if (classroom.id === items.classroomId) {
                  const teacher = users.map((user: any) => user.id === classroom.userId && user.username.toUpperCase());
                  return (
                    <span key={classroom.id} className="w-auto select-all text-start">
                      {teacher || 'No teacher name found.'}
                    </span>
                  );
                }
                return null; // No matching classroom
            ...
            {/* <div className="flex items-center justify-start gap-2 py-1.5 hover:bg-primary-foreground">
              <p className="flex flex-row text-center">Teacher Name: </p>
              {classrooms.map((classroom: any) => classroom.id === items.classroomId && <span key={classroom.id} className="w-auto select-all text-start">{users.map((user:any) => user.id === classroom.userId && user.username)}</span>)}
            </div> */}
          </div>)}
        </div>
      </div>
      <div className='fixed z-50 top-10 right-5 flex items-start justify-center w-[340px] bg-background text-sm rounded-md'>
        <div className='rounded-md p-3 w-full'>
          {submissions.map((items: any) => items.id === params.slug && <div key={items.id}>
            <div className="flex items-center justify-start gap-2 py-1.5 hover:bg-primary-foreground">
              <p className="flex flex-row text-center">Project Instructions: </p>
              <span className="w-auto select-all text-start">{items.description || "No instructions is Provided."}</span>
            </div>
          </div>)}
        </div>
      </div>
      ...
      {/* <div onClick={() => {
        setSubmitBar(!submitBar);
      }} className='w-[450px] bg-background p-3 rounded-md border text-center hover:bg-primary-foreground hover:text-primary fixed !bottom-5 left-1/2 transform -translate-x-1/2 !bg-[#FDD5B1] !hover:bg-[#f78d31] !text-[#000000] !hover:text-white'>{submitBar ? "Close" : "View"} Details</div> */}
