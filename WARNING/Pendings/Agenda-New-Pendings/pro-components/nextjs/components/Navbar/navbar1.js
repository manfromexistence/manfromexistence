import Link from 'next/link'
import Head from 'next/head'




function Navbar8() {
    return(<>
        
  {/*  Gift 🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉 */}
  {/* /////////////////////////  Nav1  //////////////////////////// */}
  <nav className="pro_navbar_main_container" id="navbar1">
    <div className="pro_navbar_mb_sm">
      <div className="pro_navbar_mb_header" id="banners">
        <h1 className="pro_navbar_brand_mb">Thapapay</h1>
        <span className="pro_navbar_mb_btn">
          <ion-icon id="icon_menu" name="menu-outline" />
          <ion-icon id="icon_close" name="close-outline" />
        </span>
      </div>
      <div className="pro_navbar_mb_main-content">
        {/* Page📃📃📃 */}
        <ul className="pro_navbar_page-menu_mb" id="circle">
          <li>
            <div className="pro_toggle_navbar_mb">
              {/* <img id="goal" src="data:image/png;base64,"/> */}
              <ion-icon name="home-outline" className="mb_icons" />
              <span className="mb_items">Home</span>
            </div>
          </li>
          <li>
            <div className="pro_toggle_navbar_mb">
              <ion-icon name="logo-tiktok" className="mb_icons" />
              <span className="mb_items">Sorts</span>
            </div>
          </li>
          <li>
            <div className="pro_toggle_navbar_mb">
              <ion-icon name="chatbubbles-outline" className="mb_icons" />
              <span className="mb_items">Messages</span>
            </div>
          </li>
          <li>
            <div className="pro_toggle_navbar_mb">
              <ion-icon name="notifications-outline" className="mb_icons" />
              <span className="mb_items">Notifications</span>
            </div>
          </li>
          <li>
            <div className="pro_toggle_navbar_mb">
              <ion-icon name="notifications-outline" className="mb_icons" />
              <span className="mb_items">Notifications</span>
            </div>
          </li>
        </ul>
        {/* Scrolling📜📜📜 op  li.scroll_li*20>a[href=#].scroll_li_a{ Page$ }*/}
        <div className="sidebar-sm">
          <div className="scroll">
                
                
            <li className="scroll_li">
                <Link href="/"><a href="" className="scroll_li_a"> Index </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/goals"><a href="" className="scroll_li_a"> Goals </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/addpost"><a href="" className="scroll_li_a"> Adpt </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/addvideo"><a href="" className="scroll_li_a"> Advo </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/about"><a href="" className="scroll_li_a"> About </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/contract"><a href="" className="scroll_li_a"> Cont </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/django"><a href="" className="scroll_li_a"> Django </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="upload2"><a href="" className="scroll_li_a"> Up2 </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/fileupload1"><a href="" className="scroll_li_a"> Filiup1 </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/home"><a href="" className="scroll_li_a"> Home </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/login"><a href="" className="scroll_li_a"> Login </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/library"><a href="" className="scroll_li_a"> Library </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/profile1"><a href="" className="scroll_li_a"> Profile1 </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/profile2"><a href="" className="scroll_li_a"> Profile2 </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/register1"><a href="" className="scroll_li_a"> Reg1 </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/register2"><a href="" className="scroll_li_a"> Reg2 </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/subscription"><a href="" className="scroll_li_a"> Suscrib </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/web3"><a href="" className="scroll_li_a"> Web3 </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/upload"><a href="" className="scroll_li_a"> Upload </a></Link>
            </li>
            <li className="scroll_li">
                <Link href="/reupload1"><a href="" className="scroll_li_a"> Reupl1 </a></Link>
            </li>
            
            
            
            <div id="marker" />
          </div>
          <div id="line" />
        </div>
      </div>
    </div>
  </nav>
  {/* /////////////////////////  End  ///////////////////////////*/}
  {/*////////////////////////////// Nav2 ///////////////////////////*/}
  <nav className="navbar navbar-expand-md navbar-dark fixed-top bg-dark" id="navbar2">
    <div className="container-fluid">
      <a className="navbar-brand" href="#">Emon</a>
      <div className="collapse navbar-collapse" id="navbarCollapse">
        <ul className="navbar-nav me-auto mb-2 mb-md-0">
          <li className="nav-item">
            <link href="/" />
            <a className="nav-link active" aria-current="page">Home</a>
          </li>
          <li className="nav-item">
            <link href="/redux" />
            <a className="nav-link active" aria-current="page">Redux</a>
          </li>
          <li className="nav-item">
            <link href="/login" />
            <a className="nav-link active" aria-current="page">Login</a>
          </li>
          <li className="nav-item">
            <link href="/django" />
            <a className="nav-link active" aria-current="page">Django</a>
          </li>
          <li className="nav-item">
            <link href="/goals" />
            <a className="nav-link active" aria-current="page">Goals</a>
          </li>
        </ul>
        <form className="d-flex">
          <input className="form-control me-2" type="search" placeholder="Search" aria-label="Search" />
          <button className="btn btn-outline-primary" type="submit">Search</button>
        </form>
      </div>
    </div>
  </nav>
    {/* /////////////////////////  End  ///////////////////////////*/}
        
        
        
        
        
        
        
        
        
        
    
    </>);
}


export default Navbar8;



