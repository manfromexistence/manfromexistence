...
We've added new components to Freelance Stuffs and made a lot of improvements to the CLI.
...
Another one by [emilkowalski\_](https://twitter.com/emilkowalski_). The last toast component you'll ever need. Sonner is now availabe in Freelance Stuffs.
...
This means you can now easily add Freelance Stuffs components to existing projects like Docusaurus, Nextra...etc. A drop-in for your existing design system with no conflict. 🔥
...
```bash
npx manofexistence-ui@latest init
```
...
```bash
npx manofexistence-ui@latest add
```
...
```bash
npx manofexistence-ui diff alert
```
